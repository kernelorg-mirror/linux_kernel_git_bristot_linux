...
void __dl_server_reapply_params(struct sched_dl_entity *dl_se, struct rq *rq)
{
	u64 new_bw = to_ratio(rq->fair_server.dl_period, rq->fair_server.dl_runtime);
	struct dl_bw *dl_b;
	int cpu = cpu_of(rq);
...
enqueue_dl_entity(struct sched_dl_entity *dl_se, int flags)
{
	 ...
	 * deffered task and still got to wait, don't enqueue.
	 */
